...
APP_TAGLINE = "Create elegant client quotations with a guided, premium, beginner-friendly experience."
APP_SUBTEXT = "Built for beauty artists who want polished quotations, smooth workflows, and a luxury brand feel without any technical confusion."
...
THEME_MOODS = {
    "Luxury Blush": "Feminine luxury",
    "Classic Ivory": "Timeless bridal",
    "Modern Minimal": "Clean professional",
    "Royal Gold": "Rich premium",
    "Soft Floral": "Romantic soft",
    "Bold Editorial": "Statement look",
    "Bridal Beige": "Warm classic",
    "Rose Noir": "Boutique luxe",
    "Pastel Glam": "Fresh stylish",
    "Signature Studio": "Modern signature",
}
...
def build_quote_html_cached(quote_signature_text: str):
    ...
    <body style="background:#f8f5f7;padding:16px;margin:0;">
        <div style="max-width:920px;margin:auto;background:{theme['bg']};border:1px solid {theme['soft']};border-radius:26px;padding:30px;font-family:{theme['font']};color:{theme['text']};box-shadow:0 18px 50px rgba(0,0,0,0.08);">
            ...
            <table style="width:100%;border-collapse:collapse;border-bottom:2px solid {theme['soft']};margin-bottom:20px;">
                <tr>
                    ...
                    <td style="width:38%;vertical-align:top;text-align:right;padding:0 0 18px 12px;">
                        <div style="display:inline-block;background:{theme['soft']};padding:8px 14px;border-radius:999px;font-size:12px;font-weight:700;color:{theme['accent']};margin-bottom:10px;">Quotation</div>
                        <p style="margin:8px 0 0 0;line-height:1.8;">
                            <strong>Theme:</strong> {html.escape(data['selected_template'])}<br>
                            <strong>Quote No:</strong> {html.escape(data['quote_number'])}<br>
                            <strong>Date:</strong> {html.escape(data['quote_date'])}<br>
                            <strong>Valid Till:</strong> {html.escape(data['valid_till'])}
                        </p>
                    </td>
                </tr>
            </table>
            ...
            <table style="width:100%;border-collapse:separate;margin-bottom:20px;">
                <tr>
                    <td style="width:50%;vertical-align:top;padding-right:11px;">
                        <div style="background:{theme['soft']};border-radius:20px;padding:18px;min-height:132px;">
            ...
            <table style="width:100%;border-collapse:collapse;margin-bottom:20px;border-radius:16px;overflow:hidden;background:white;">
                <thead>
            ...
            <table style="width:100%;border-collapse:separate;">
                <tr>
                    ...
                    <td style="width:40%;vertical-align:top;padding-left:11px;">
                        <div style="background:{theme['soft']};border-radius:20px;padding:20px;">
...
# Reusable UI blocks
# ==========================================================
def render_luxury_header(logged_in: bool):
    status_text = "Signed in and ready to create premium quotations." if logged_in else "Create your account and start building luxury client quotations."
    c1, c2 = st.columns([1.55, 1])
    with c1:
        st.markdown(
            f"""
            <div class="lux-hero fade-up">
                <div class="lux-badge">Luxury quotation experience</div>
                <h1>{APP_TITLE}</h1>
                <p class="lux-subtitle">{APP_TAGLINE}</p>
                <p class="lux-copy">{APP_SUBTEXT}</p>
            </div>
            """,
    ...
    with c2:
        st.markdown(
            f"""
            <div class="lux-side-card fade-up delay-1">
                <div class="lux-mini-title">Why this version feels better</div>
                <div class="lux-list-item">• Softer premium styling with subtle motion</div>
                <div class="lux-list-item">• Cleaner visual hierarchy for first-time users</div>
                <div class="lux-list-item">• Mobile-first stacked flow for easier form filling</div>
                <div class="lux-list-item">• Branding that matches a luxury beauty business</div>
                <div class="lux-status">{status_text}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def render_quick_stats(logged_in: bool):
    if logged_in and st.session_state.get("quote_totals"):
        totals = st.session_state["quote_totals"]
        a, b, c = st.columns(3)
        a.metric("Current quotation total", format_inr(totals["grand_total"]))
        b.metric("Balance due", format_inr(totals["balance_due"]))
        c.metric("Services added", str(len([s for s in st.session_state.get("quote_data", {}).get("services", []) if str(s.get("name", "")).strip()])))
    else:
        a, b, c = st.columns(3)
        a.metric("Beautiful themes", "10")
        b.metric("Saved quotations", "Fast access")
        c.metric("PDF export", "One click")



def render_section_intro(title: str, subtitle: str, badge: str = ""):
    badge_html = f'<span class="lux-inline-badge">{badge}</span>' if badge else ""
    st.markdown(
        f"""
        <div class="lux-section-head fade-up">
            <div class="lux-section-topline">{badge_html}</div>
            <h3>{title}</h3>
            <p>{subtitle}</p>
        </div>
        """,
...
def render_theme_gallery(current_theme: str):
    render_section_intro(
        "Quotation theme gallery",
        "Choose a look that feels right for your brand. Each preview card shows the quotation style in a more visual and beginner-friendly way.",
        "Branding",
    )
    ...
    for idx, template in enumerate(TEMPLATE_OPTIONS):
        ...
        selected_badge = "<span class='theme-current'>Current default</span>" if template == current_theme else ""
        with cols[idx % 2]:
            st.markdown(
                f"""
                <div class="theme-card fade-up delay-{(idx % 3) + 1}" style="background:{theme['bg']};border-color:{theme['soft']};color:{theme['text']};font-family:{theme['font']};">
                    <div class="theme-accent" style="background:linear-gradient(90deg,{theme['accent']}, {theme['soft']});"></div>
                    <div class="theme-header-row">
                        <div>
                            <div class="theme-title">{template}</div>
                            <div class="theme-mood">{THEME_MOODS.get(template, '')}</div>
                        </div>
                        <div class="theme-dot" style="background:{theme['accent']};"></div>
                    </div>
                    <div class="theme-description">{THEME_DESCRIPTIONS.get(template, '')}</div>
                    <div class="theme-mini-quote">
                        <div class="theme-mini-head">
                            <span>MakeupByRuchi</span>
                            <span>Quotation</span>
                        </div>
                        <div class="theme-mini-body">
                            <div>Client: Priya Shah</div>
                            <div>Event: Bridal Makeup</div>
                            <div>Package: Signature Bridal Look</div>
                        </div>
                        <div class="theme-mini-total" style="background:{theme['soft']};">
                            <span>Total</span>
...
def render_quote_preview_and_actions(quote_data: dict, user_id: str):
    render_section_intro(
        ...
        "Review the design below, then save it or prepare the PDF only when you need it. This keeps the app smooth and faster.",
        "Preview",
    )
    ...
    m3.metric("Services included", str(len([s for s in quote_data.get("services", []) if str(s.get("name", "")).strip()])))

    st.components.v1.html(st.session_state["quote_html"], height=PREVIEW_HEIGHT, scrolling=True)

    st.markdown("<div class='action-shell fade-up'>", unsafe_allow_html=True)
    c1, c2, c3, c4 = st.columns([1.15, 1, 1.1, 0.95])
...
def render_saved_quote_actions(quote_json: dict, idx: int):
    ...
    with a1:
        if st.button("Open in builder", key=f"open_quote_{idx}", use_container_width=True):
            set_active_quote(quote_json)
            st.success("This quotation is now loaded in the Create Quotation tab.")

    with a2:
        if st.button("Duplicate & edit", key=f"duplicate_quote_{idx}", use_container_width=True):
            set_active_quote(quote_json)
            st.success("A copy of this quotation is now ready for editing.")
...
:root {
    ...
    --lux-muted: #7c6570;
    --lux-accent: #b76e79;
    --lux-accent-soft: #f4e8ec;
    --lux-gold: #c8a96b;
    --lux-shadow: 0 16px 42px rgba(110, 72, 82, 0.10);
}
html, body, [class*="css"] {
...
.stApp {
    background:
        radial-gradient(circle at top left, rgba(248, 226, 233, 0.8), transparent 28%),
        radial-gradient(circle at top right, rgba(253, 241, 226, 0.65), transparent 22%),
        linear-gradient(180deg, #fffafc 0%, #fbf7f8 100%);
}
.block-container {
    padding-top: 1.15rem;
...
.lux-hero, .lux-side-card, .lux-shell, .theme-card, .saved-card {
...
.lux-badge, .lux-inline-badge, .theme-current {
    display: inline-flex;
    align-items: center;
    gap: 6px;
    padding: .38rem .8rem;
...
.lux-status {
    ...
    padding: .85rem 1rem;
    border-radius: 16px;
    background: linear-gradient(135deg, rgba(183,110,121,.10), rgba(200,169,107,.10));
    color: var(--lux-text);
    font-weight: 600;
}
.lux-section-head {
...
.theme-card:hover, .saved-card:hover {
    transform: translateY(-2px);
    box-shadow: 0 20px 46px rgba(110,72,82,0.13);
}
.theme-accent {
    height: 8px;
    border-radius: 999px;
...
.theme-mini-head, .theme-mini-total {
    display:flex;
    justify-content:space-between;
    align-items:center;
}
...
.theme-current {
    margin-top: 12px;
    background: rgba(29, 125, 81, 0.10);
    color: #156542;
    border: 1px solid rgba(29,125,81,.12);
}
.action-shell {
    margin-top: .5rem;
}
...
.stTabs [aria-selected="true"] {
    background: linear-gradient(135deg, rgba(244,233,237,.95), rgba(255,247,239,.95)) !important;
    border-color: rgba(183,110,121,.20) !important;
}
...
.stButton > button, .stDownloadButton > button {
    ...
    box-shadow: 0 8px 18px rgba(183,110,121,0.08);
}
.stButton > button:hover, .stDownloadButton > button:hover {
    transform: translateY(-1px);
    border-color: rgba(183,110,121,.34);
    box-shadow: 0 14px 28px rgba(183,110,121,0.14);
}
...
# Page header
# ==========================================================
render_luxury_header(logged_in=bool(st.session_state.logged_in and st.session_state.user))
render_quick_stats(logged_in=bool(st.session_state.logged_in and st.session_state.user))
...
if (not st.session_state.logged_in) or (not st.session_state.user):
    ...
    render_section_intro(
        "Welcome",
        "Start by signing in or creating your account. The flow below is written in simple language so first-time users can move through it easily.",
        "Start here",
    )
    ...
    with login_tab:
        render_section_intro(
            ...
            "Use your registered email and password. If you are new, create your account first and verify your email once.",
            "Account access",
        )
    ...
    with register_tab:
        render_section_intro(
            "Create your account",
            "Once your account is created, open your email and click the verification button before signing in.",
            "New account",
        )
        ...
        render_section_intro(
            ...
            "Please check Promotions and Spam too. If needed, send the verification email again below.",
            "Verification help",
        )
...
# ==========================================================
    ...
    st.sidebar.info("Use the tabs below to update your business details, create quotations, and reopen saved work.")
    st.sidebar.markdown("---")
    st.sidebar.caption("Luxury brand note")
    st.sidebar.write("Keep your logo, default services, and preferred theme updated so every quotation feels consistent.")
    ...
    setup_tab, quote_tab, history_tab, account_tab = st.tabs([
        "My Business Details",
        "Create Quotation",
        "Saved Quotations",
        "Account & Help",
    ...
    with setup_tab:
        render_section_intro(
            ...
            "Save these details once so future quotations automatically feel consistent, polished, and on-brand.",
            "Foundation",
        )

        with st.container(border=False):
            st.markdown("<div class='lux-shell fade-up'>", unsafe_allow_html=True)
            st.subheader("Business identity")
            st.caption("These details appear across your quotations and help the app feel more premium and consistent.")
        ...
        with st.container(border=False):
            st.markdown("<div class='lux-shell fade-up delay-1'>", unsafe_allow_html=True)
            st.subheader("Branding")
            st.caption("Upload your logo and choose the default quotation look you want your clients to see most often.")
            ...
            st.caption(f"Mood: {THEME_MOODS.get(selected_template, '')} • {THEME_DESCRIPTIONS.get(selected_template, '')}")
            st.markdown("</div>", unsafe_allow_html=True)

        with st.container(border=False):
            st.markdown("<div class='lux-shell fade-up delay-2'>", unsafe_allow_html=True)
    ...
    with quote_tab:
        render_section_intro(
            ...
            "This flow is now more mobile-friendly: fill the form from top to bottom, generate the preview, and review everything below.",
            "Main workflow",
        )
        ...
        with st.form("quote_form"):
            ...
            st.caption(f"Mood: {THEME_MOODS.get(chosen_template, '')} • {THEME_DESCRIPTIONS.get(chosen_template, '')}")
        ...
        if not st.session_state.quote_data:
            st.info("Fill in the details above and click Generate quotation preview. Your styled quotation will appear below.")
    ...
    with history_tab:
        render_section_intro(
            "Saved quotations",
            "Search, reopen, duplicate, or download any quotation you have already saved. This section now feels more like a premium mini-dashboard.",
            "History",
        )
        ...
        try:
            ...
            if not recent_quotes:
                ...
                for idx, row in enumerate(recent_quotes):
                    ...
                    st.markdown("<div class='saved-card fade-up'>", unsafe_allow_html=True)
                    st.markdown(f"### {row.get('quote_number', 'No quotation number')} • {row.get('client_name', 'No client name')}")
                    st.markdown(
                        f"<div class='saved-meta'><strong>Client:</strong> {row.get('client_name', '')}<br>"
                        f"<strong>Phone:</strong> {row.get('client_phone', '')}<br>"
                        f"<strong>Event:</strong> {row.get('event_type', '')}<br>"
                        f"<strong>Event date:</strong> {row.get('event_date', '')}<br>"
                        f"<strong>Package:</strong> {row.get('package_name', '')}<br>"
                        f"<strong>Theme:</strong> {row.get('selected_template', '')}<br>"
                        f"<strong>Grand total:</strong> {format_inr(safe_float(row.get('grand_total', 0)))}<br>"
                        f"<strong>Saved on:</strong> {row.get('created_at', '')}</div>",
                        unsafe_allow_html=True,
                    )
    ...
    with account_tab:
        render_section_intro(
            "Account and help",
            "Use this section for password help, verification guidance, and support for first-time users.",
            "Support",
        )
        ...
        with st.container(border=False):
            st.markdown("<div class='lux-shell fade-up delay-1'>", unsafe_allow_html=True)
            st.subheader("Verification email help")
