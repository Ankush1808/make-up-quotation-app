...
def render_theme_gallery(current_theme: str):
    ...
    st.caption("Each theme below shows the overall look and feel of your quotation.")
...
def render_quote_preview_and_actions(quote_data: dict, user_id: str):
    ...
    st.caption("Preview is cached for speed. PDF is prepared only when you ask for it.")
    ...
    with c1:
        if st.button("Save quotation to database", key="save_db_current"):
            try:
    ...
    with c2:
        if st.button("Prepare PDF", key="prepare_pdf_current"):
    ...
    with c3:
        ...
        if pdf_bytes:
            ...
            st.caption("Click Prepare PDF once, then the download button will appear here.")
...
def render_saved_quote_actions(quote_json: dict, idx: int):
    ...
    with a2:
        if st.button("Prepare PDF for this saved quotation", key=f"prepare_hist_pdf_{idx}"):
            with st.spinner("Preparing PDF..."):
    ...
    show_preview = st.checkbox("Show full preview", key=preview_key)
    if show_preview:
...
if (not st.session_state.logged_in) or (not st.session_state.user):
    ...
    with login_tab:
        st.info("If this is your first time, create your account, verify your email, and then log in here.")
        ...
        if reset_btn:
            try:
                ...
                st.info("For the smoothest reset flow, keep your app URL updated in Supabase redirect settings and in APP_BASE_URL.")
            except Exception as exc:
    ...
    with register_tab:
        ...
        if reg_btn:
            try:
                ...
                st.info("A verification email has been sent. Please open your email, click the verify button, then come back here and log in.")
            except Exception as exc:
...
# ==========================================================
    ...
    st.sidebar.info("Your account details and saved quotations are stored in your database.")
    ...
    with setup_tab:
        ...
        with left:
            st.subheader("Your brand details")
        ...
        with right:
            st.subheader("Quotation form settings")
    ...
    with history_tab:
        ...
        st.caption("You can reopen any saved quotation and download it again when needed.")
