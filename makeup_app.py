...
def render_quote_preview_and_actions(quote_data: dict, section_key: str, user_id: str):
    ...
    with c2:
        if pdf_bytes:
            file_name = f"{re.sub(r'[^A-Za-z0-9_-]+', '-', quote_data.get('quote_number', 'quotation')).strip('-') or 'quotation'}.pdf"
            st.download_button(
                "Download PDF",
                data=pdf_bytes,
                file_name=file_name,
...
# ==========================================================
    ...
    with history_tab:
        ...
        st.caption("You can reopen any saved quotation and download it again anytime.")
                ...
                for idx, row in enumerate(recent_quotes):
                    ...
                    with st.expander(heading):
                        ...
                        with a2:
                            ...
                            if pdf_bytes:
                                file_name = f"{re.sub(r'[^A-Za-z0-9_-]+', '-', quote_json.get('quote_number', 'quotation')).strip('-') or 'quotation'}.pdf"
                                st.download_button(
                                    "Download PDF again",
                                    data=pdf_bytes,
                                    file_name=file_name,
