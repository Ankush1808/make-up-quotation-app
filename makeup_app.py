...
APP_TITLE = "💄 Makeup Quotation App"
APP_TAGLINE = "Create simple, beautiful client quotations without any technical steps."
DEFAULT_WHATSAPP_TEXT = "Hello {client_name}, please find your quotation here: {pdf_url}"
DEFAULT_STORAGE_BUCKET = "quotation-pdfs"
...
def get_storage_bucket_name() -> str:
    return str(st.secrets.get("SUPABASE_STORAGE_BUCKET", DEFAULT_STORAGE_BUCKET)).strip()
...
def sanitize_filename(text: str) -> str:
    cleaned = re.sub(r"[^A-Za-z0-9_-]+", "-", (text or "file").strip())
...
# ==========================================================
# Storage helpers for PDF sharing
# ==========================================================
def upload_pdf_and_get_public_url(user_id: str, quote_data: dict, pdf_bytes: bytes):
    bucket = get_storage_bucket_name()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    quote_number = sanitize_filename(quote_data.get("quote_number") or "quotation")
    client_name = sanitize_filename(quote_data.get("client_name") or "client")
    file_path = f"{user_id}/{client_name}_{quote_number}_{timestamp}.pdf"
...
def build_quote_html(data, totals):
    ...
    if data.get("show_travel_charges"):
        ...
        <tr>
            ...
            <td style="padding:12px;text-align:right;">{format_inr(safe_float(data['travel_charges']))}</td>
        </tr>
        """
    if data.get("show_extra_charges"):
        ...
        <tr>
            ...
            <td style="padding:12px;text-align:right;">{format_inr(safe_float(data['extra_charges']))}</td>
        </tr>
        """
    if data.get("show_discount"):
        ...
        <tr>
            ...
            <td style="padding:12px;text-align:right;">- {format_inr(safe_float(data['discount']))}</td>
        </tr>
        """

    terms_html = "".join(
        f"<li style='margin-bottom:6px;'>{html.escape(str(term))}</li>"
    ...
    if data.get("show_advance_paid"):
        payment_rows.append(f"<strong>Advance Paid:</strong> {format_inr(safe_float(data['advance_paid']))}<br>")
            ...
            <table style="width:100%;border-collapse:collapse;border-bottom:2px solid {theme['soft']};margin-bottom:24px;">
                <tr>
                    <td style="width:62%;vertical-align:top;padding:0 12px 18px 0;">
                        {logo_html}
                        <h1 style="margin:0;font-size:30px;color:{theme['accent']};line-height:1.1;">{html.escape(data['business_name'])}</h1>
...
def generate_pdf_reportlab(data: dict, totals: dict):
    try:
        ...
        right_text_style = ParagraphStyle(
            "RightTextStyle", parent=normal, alignment=2, fontSize=10, leading=14
        )
        ...
        artist_lines = [
            html.escape(data.get("artist_name", "")),
            html.escape(data.get("contact", "")),
            html.escape(data.get("email", "")),
        ]
        ...
        right_parts = [
            ...
            Paragraph(
                "<br/>".join(
                    [
                        f"Theme: {html.escape(data.get('selected_template', ''))}",
                        f"Quote No: {html.escape(data.get('quote_number', ''))}",
                        f"Date: {html.escape(data.get('quote_date', ''))}",
                        f"Valid Till: {html.escape(data.get('valid_till', ''))}",
                    ]
                ),
                right_text_style,
            ),
...
# Reusable UI blocks
# ==========================================================
def render_theme_gallery(current_theme: str, section_key: str):
    st.markdown("### Theme Preview Gallery")
    st.caption("Each theme below shows the overall feel of the quotation, not just the color.")
    ...
    for idx, template in enumerate(TEMPLATE_OPTIONS):
        ...
        with cols[idx % 2]:
            st.markdown(
                ...
                ">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
                        ...
                        <div style="background:{theme['accent']};width:28px;height:28px;border-radius:50%;"></div>
                    </div>
                    <div style="border:1px solid {theme['soft']};background:white;border-radius:16px;padding:12px;">
                        <div style="font-size:15px;color:{theme['accent']};font-weight:700;">Quotation Preview</div>
                        <div style="margin-top:6px;font-size:12px;line-height:1.6;">
                            Client: Priya Shah<br>
                            Package: Bridal Makeup<br>
                            Total: ₹28,000
                        </div>
                        <div style="margin-top:10px;background:{theme['soft']};padding:8px 10px;border-radius:12px;font-size:12px;">
                            {THEME_DESCRIPTIONS.get(template, '')}
                        </div>
                    </div>
                </div>
                """,
            ...
            if template == current_theme:
                st.success(f"Selected right now: {template}")


def render_quote_preview_and_actions(quote_data: dict, section_key: str, user_id: str):
    totals = calc_totals(
        quote_data["services"],
    ...
    html_content = build_quote_html(quote_data, totals)
    pdf_bytes, pdf_error = generate_pdf_reportlab(quote_data, totals)

    st.subheader("Quotation Preview")
    st.components.v1.html(html_content, height=1180, scrolling=True)

    c1, c2, c3 = st.columns(3)

    with c1:
        if st.button("Save quotation to database", key=f"save_db_{section_key}"):
            try:
                save_quotation(user_id, quote_data, totals)
                st.success("Quotation saved to database.")
            except Exception as exc:
                st.error(f"Could not save quotation: {exc}")
    ...
    with c2:
        if pdf_bytes:
            st.download_button(
                ...
                key=f"download_pdf_{section_key}",
            )
        else:
            st.error(f"PDF generation failed: {pdf_error}")

    with c3:
        render_whatsapp_share_block(quote_data, pdf_bytes, section_key, user_id)


def render_whatsapp_share_block(quote_data: dict, pdf_bytes: bytes, section_key: str, user_id: str):
    st.markdown("**Share to client on WhatsApp**")
    phone = clean_phone_for_whatsapp(quote_data.get("client_phone", ""))

    if not phone:
        st.caption("Enter the client's phone number to enable WhatsApp sharing.")
        return

    if not pdf_bytes:
        st.caption("Generate a valid quotation first.")
        return

    try:
        if st.button("Create WhatsApp share link", key=f"wa_share_{section_key}"):
            pdf_url, _ = upload_pdf_and_get_public_url(user_id, quote_data, pdf_bytes)
            share_text = DEFAULT_WHATSAPP_TEXT.format(
                client_name=quote_data.get("client_name", "Client"),
                pdf_url=pdf_url,
            )
            wa_link = f"https://wa.me/{phone}?text={quote(share_text)}"
            st.success("WhatsApp link is ready.")
            st.markdown(f"[Open WhatsApp chat]({wa_link})")
            st.caption(
                "This sends the client a PDF link directly in WhatsApp. "
                "The actual file is not attached inside WhatsApp because browser-based click-to-chat supports text links, not direct document attachments."
            )
    except Exception as exc:
        st.error(
            "WhatsApp share could not be prepared. Make sure your Supabase Storage bucket exists and is public. "
            f"Details: {exc}"
        )
...
if (not st.session_state.logged_in) or (not st.session_state.user):
    ...
    with login_tab:
        st.info(
            "If this is your first time, create your account, verify your email, and then log in here."
        )
        ...
        st.caption("Enter your email below and we will send a password reset email.")
        with st.form("forgot_password_form"):
            reset_email = st.text_input("Your registered email", key="reset_email")
            reset_btn = st.form_submit_button("Send password reset email")
        ...
        if reset_btn:
            try:
                ...
                st.info(
                    "Important: for the smoothest reset flow, set your app URL in Supabase Auth redirect settings and in APP_BASE_URL."
                )
            except Exception as exc:
                st.error(f"Could not send reset email: {exc}")

        st.markdown("### Need the verification email again?")
        with st.form("resend_verify_form"):
            verify_email = st.text_input("Email for verification link", key="verify_email")
            resend_btn = st.form_submit_button("Resend verification email")

        if resend_btn:
            try:
                resend_verification_email(verify_email.strip())
    ...
    with register_tab:
        ...
        st.caption("This takes less than a minute.")
        ...
        if reg_btn:
            try:
                ...
                if user:
                    ...
                    st.info(
                        "A verification email has been sent. Please open your inbox, click the verification link, then return here and log in."
                    )
                    st.warning(
                        "If the link opens a plain page or your app does not load after clicking it, your email may still be verified successfully. "
                        "Come back to this app and try logging in. For the cleanest experience, set your production app URL in Supabase Auth settings."
                    )
                else:
                    st.info("Signup request submitted. Please verify your email before logging in.")
            except Exception as exc:
...
# ==========================================================
    ...
    st.sidebar.info("Your profile and quotations are stored in the database.")
    ...
    setup_tab, quote_tab, history_tab = st.tabs([
        "Artist setup",
        "Create quotation",
        "Recent quotations",
    ])
    ...
    with setup_tab:
        ...
        with left:
            ...
            st.caption("These details will appear automatically on every quotation.")
        ...
        with right:
            st.subheader("What should appear on quotation forms?")
            st.caption("Turn fields on or off based on how you work with clients.")
            show_travel_charges = st.checkbox("Show Travel Charges", value=form_config.get("show_travel_charges", True))
            show_extra_charges = st.checkbox("Show Extra Charges", value=form_config.get("show_extra_charges", True))
            show_discount = st.checkbox("Show Discount", value=form_config.get("show_discount", True))
            show_advance_paid = st.checkbox("Show Advance Paid", value=form_config.get("show_advance_paid", True))

            st.markdown("#### Default services")
            st.caption("These service lines will appear first whenever you create a new quotation.")
            existing_services = form_config.get("service_options", DEFAULT_SERVICES)
            service_count = st.number_input(
                "How many default services would you like?",
                min_value=1,
                max_value=12,
                value=len(existing_services),
                step=1,
            )
            custom_services = []
            for i in range(int(service_count)):
                ...
                with c1:
                    srv_name = st.text_input(f"Service name {i + 1}", value=default_name, key=f"cfg_srv_name_{i}")
                with c2:
            ...
            st.markdown("#### Default terms and conditions")
        ...
        render_theme_gallery(selected_template, "setup")

    with quote_tab:
        st.subheader("Create a quotation")
        st.caption("Fill the client details on the left. Your quotation preview will appear on the right.")
        ...
        with left_col:
            with st.form("quote_form"):
                ...
                service_count = st.number_input(
                    "How many services do you want to show?",
                    min_value=1,
                    max_value=12,
                    value=len(default_services),
                    step=1,
                )
                services = []
                for i in range(int(service_count)):
                ...
                st.markdown("### Additional pricing")
            ...
            if generate:
                ...
                st.session_state.quote_data = build_quote_data_from_form(profile, form_config, user_email, form_values)

        with right_col:
            if not st.session_state.quote_data:
                st.info("Fill the form and click Generate quotation to see your preview here.")
                render_theme_gallery(current_template, "quote_empty")
            else:
                render_quote_preview_and_actions(st.session_state.quote_data, "current", user_id)

    with history_tab:
        st.subheader("Recent quotations")
        st.caption("You can reopen any saved quotation, download it again, and prepare a WhatsApp share link.")
        ...
        try:
            ...
            if not recent_quotes:
                ...
                for idx, row in enumerate(recent_quotes):
                    ...
                    totals = calc_totals(
                        quote_json.get("services", []),
                        quote_json.get("travel_charges", 0),
                        quote_json.get("extra_charges", 0),
                        quote_json.get("discount", 0),
                        quote_json.get("advance_paid", 0),
                    )
                    ...
                    with st.expander(heading):
                        ...
                        with a1:
                            if st.button("Open this quotation", key=f"open_quote_{idx}"):
                                st.session_state.quote_data = quote_json
                                st.success("Quotation loaded in the Create quotation tab.")
                        with a2:
                            pdf_bytes, pdf_error = generate_pdf_reportlab(quote_json, totals)
                            if pdf_bytes:
                                st.download_button(
                                    "Download PDF again",
                                    data=pdf_bytes,
                                ...
                                st.error(f"Could not prepare PDF: {pdf_error}")
                        with a3:
                            if pdf_bytes:
                                render_whatsapp_share_block(quote_json, pdf_bytes, f"history_{idx}", user_id)

                        html_preview = build_quote_html(quote_json, totals)
                        st.components.v1.html(html_preview, height=850, scrolling=True)
        except Exception as exc:
            st.error(f"Could not load quotation history: {exc}")
